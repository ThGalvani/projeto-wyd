...
void Exec_MSG_TradingItem(int conn, char *pMsg)
{
	...
	{
		SendClientMessage(conn, "Imposs�vel executar a��o52, tente mais tarde. ");
		return;
	}
	...
	{
		SendClientMessage(conn, "Passe o item para o invent�rio antes de equip�-lo.");
		return;
	}
	...
	if (m->DestPlace)
	{
		...
		else
		{
			...
			if (NearCargo == false) {
				SendClientMessage(conn, "Voc� est� longe do Bau");
				return;
			}
		}
	}
